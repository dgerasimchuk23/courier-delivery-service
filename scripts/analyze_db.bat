...
REM Добавление пути PostgreSQL в PATH
set PATH=%PATH%;C:\Program Files\PostgreSQL\14\bin

REM Путь к PostgreSQL - конкретный путь, который виден на скриншоте
set PSQL_PATH=C:\Program Files\PostgreSQL\14\bin\psql.exe

REM Проверка существования psql
if not exist "%PSQL_PATH%" (
    echo Ошибка: Файл %PSQL_PATH% не найден
    echo Проверьте путь к PostgreSQL
    exit /b 1
)
...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "SELECT schemaname, tablename, indexname, indexdef FROM pg_indexes WHERE schemaname = 'public' ORDER BY tablename, indexname;" > "%RESULTS_DIR%\indexes.txt"
...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "EXPLAIN ANALYZE SELECT * FROM users WHERE email = 'test@example.com';" > "%RESULTS_DIR%\query_users_email.txt"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "EXPLAIN ANALYZE SELECT * FROM users WHERE id = 1;" > "%RESULTS_DIR%\query_users_id.txt"
...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "EXPLAIN ANALYZE SELECT * FROM refresh_tokens WHERE token = 'test-token';" > "%RESULTS_DIR%\query_refresh_tokens_token.txt"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "EXPLAIN ANALYZE SELECT * FROM refresh_tokens WHERE user_id = 1;" > "%RESULTS_DIR%\query_refresh_tokens_user_id.txt"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "EXPLAIN ANALYZE SELECT * FROM refresh_tokens WHERE expires_at < NOW();" > "%RESULTS_DIR%\query_refresh_tokens_expires.txt"
...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "SELECT pid, now() - pg_stat_activity.query_start AS duration, query, state FROM pg_stat_activity WHERE state != 'idle' AND now() - pg_stat_activity.query_start > interval '5 seconds' ORDER BY duration DESC;" > "%RESULTS_DIR%\long_queries.txt"
...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "DELETE FROM refresh_tokens WHERE expires_at < NOW();"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "DELETE FROM refresh_tokens WHERE id IN (SELECT id FROM (SELECT id, ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY created_at DESC) as row_num FROM refresh_tokens) as ranked WHERE row_num > 5);"

REM Обновление статистики для оптимизатора запросов
echo Обновление статистики для оптимизатора запросов...
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE users;"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE refresh_tokens;"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE customer;"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE courier;"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE parcel;"
start /wait "" "%PSQL_PATH%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -c "ANALYZE delivery;"
